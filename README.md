...
"대화"·"쪽지"·"워크스페이스" 세 곳 모두 브리티 자신의 "읽지 않음" 탭을 기준으로
확인합니다.

"메신저 알리미"를 켜두면, 브리티 메신저를 직접 쓰고 있지 않은 동안(자리비움 30초 이상
& 브리티 창에 포커스가 없을 때)에만 확인합니다. 세 곳을 매번 다 넘나들면 화면이 계속
바뀌는 것처럼 보여서, 확인 주기를 종류별로 다르게 뒀습니다 — **쪽지는 1분마다**,
**대화·워크스페이스는 2분마다**. 확인할 때 안 읽은 게 여러 개면 하나하나 따로 알리지
않고, 전부 읽어서 하나로 합쳐 한 번에 서버로 보냅니다. 새로 연 창은 방금 자기가 연
창만 스스로 닫습니다 — 사용자가 직접 열어둔 창은 절대 건드리지 않습니다.

**아직 안 되는 것:** "테스트 쪽지 보내기 (개발용)" 메뉴는 이 실제 리더에는 없습니다
(실제 쪽지가 오는지로 확인).
...
역할만 합니다. 실제로 화면을 확인할지 말지는 20초 주기와 무관하게 `reader.py` 안에서
아래 규칙으로 스스로 판단합니다.

- `reader.py`는 `pywinauto`(윈도우 UI Automation을 파이썬에서 쓰기 쉽게 감싼 라이브러리)로
  브리티의 "대화"·"쪽지"·"워크스페이스"(채널 뷰) 세 화면을 각각의 "읽지 않음" 탭
  기준으로 읽습니다. 각 화면을 확인하려면 그 화면으로 넘어가야 해서(포커스가 필요한
  동작), 매 20초 주기마다 세 곳을 다 넘나들지 않도록 종류별로 확인 간격을 따로 둡니다
  (`ROOM_CHECK_INTERVAL_SEC`/`CHANNEL_CHECK_INTERVAL_SEC` = 120초, `NOTE_CHECK_INTERVAL_SEC`
  = 60초) — 각자 그 간격이 지났을 때만 실제로 화면을 넘겨 확인하고, 아니면 이번 20초
  주기는 아예 아무것도 건드리지 않고 조용히 끝냅니다.
- 확인할 때 안 읽은 게 여러 개면 하나하나 따로 알리지 않고, 전부 열어서 읽은 뒤
  하나로 합쳐 한 번에 서버로 보냅니다(자잘한 대화까지 낱개로 알림이 오는 걸 막기
  위해서). 안 읽은 방/쪽지/채널만 골라 더블클릭으로 열고, 다 읽으면 자기가 연
  창만 닫습니다.
- 자리비움 30초 미만이거나 브리티 창에 포커스가 있으면(=사용자가 직접 쓰고 있으면)
  어떤 화면도 확인하지 않고 다음 20초 주기를 기다립니다.
- "본 목록"과 각 종류별 마지막 확인 시각은 `app.getPath('userData')/brity-seen.json`에
  저장됩니다.
